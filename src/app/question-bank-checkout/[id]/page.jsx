...
export default function QuestionBankCheckoutPage() {
    const { id } = useParams();
    ...
    useEffect(() => {
        async function fetchQuestionBankDetails() {
            ...
            try {
                const response = await axios.get(`/api/questionBanks/${id}`); // Assumes a GET endpoint for individual question bank
                ...
                toast.error("প্রশ্নব্যাংক লোড করতে ব্যর্থ হয়েছে।");
            } finally {
                setLoading(false);
            }
        }
    ...
    const handlePayment = async () => {
        ...
        if (alreadyPurchased) {
            toast.error("আপনার এই প্রশ্নব্যাংক ইতিমধ্যে সক্রিয় রয়েছে।");
            return;
        }
        setStep(3);
        try {
    ...
    if (!questionBankData) return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-gray-100 flex items-center justify-center">
            <p className="text-2xl text-red-600">প্রশ্নব্যাংক লোড করতে ব্যর্থ হয়েছে।</p>
        </div>
    );
            ...
            <div className="max-w-4xl mx-auto py-12 px-6">
                ...
                <div className="bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl p-8 border border-blue-100">
                    ...
                    {step === 2 && (
                        <div className="space-y-6">
                            <div className="flex flex-col items-center gap-4">
                                <h1 className="text-3xl font-bold text-indigo-900">Question Bank Details</h1>
                                <h2 className="text-2xl font-semibold">{questionBankData.name}</h2>
                                <p className="text-gray-600">Validity: {new Date(questionBankData.validity).toLocaleDateString('bn-BD')}</p>
                            </div>
                            <div className="p-6 bg-gray-50 rounded-xl">
                                <h3 className="text-lg font-semibold text-indigo-900">Payment Details</h3>
                                <div className="flex justify-between mt-4">
                                    <span className="text-gray-700">Question Bank Price</span>
                                    <span className="text-lg font-medium">৳{questionBankData.price}</span>
                                </div>
                                ...
                                <div className="flex justify-between mt-4 text-xl font-bold">
                                    ...
                                    <span className="text-indigo-900">৳{questionBankData.price}</span>
                                </div>
                            </div>
                    ...
                    {step === 3 && (
                        <div className="space-y-6 text-center">
                            ...
                            <p className="text-gray-700">
                                You have selected <strong>{paymentMethod}</strong> for ৳{questionBankData.price}. Confirm to proceed.
                            </p>
                    ...
                    {step === 4 && (
                        <div className="space-y-6 text-center">
                            ...
                            <p className="text-gray-700">Redirecting to question bank shelf...</p>
                        </div>
                    )}
                </div>
            </div>
