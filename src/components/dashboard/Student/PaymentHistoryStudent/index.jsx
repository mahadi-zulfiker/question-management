...
export default function PaymentHistoryStudent() {
    ...
    useEffect(() => {
        if (status === "authenticated") {
            fetch(`/api/paymentHistoryStudent?email=${session.user.email}`)
                .then((res) => res.json())
                .then((data) => {
    ...
    return (
        <div className="p-6 max-w-5xl mx-auto">
            <h1 className="text-3xl font-bold mb-6 text-center text-gray-800">💳 Student Payment History</h1>
                        ...
                        {payments.length > 0 ? (
                            payments.map((payment) => (
                                <tr key={payment._id} className="border-b hover:bg-gray-50 transition-all">
                                    <td className="p-3 text-gray-800">{payment._id.toString().slice(-6)}</td>
