...
export default function CreateModelTest() {
  ...
  const fetchAllClasses = async () => {
    setIsLoading(true);
    try {
      const res = await fetch('/api/createModelTest');
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const url = `/api/createModelTest${query.length ? '?' + query.join('&') : ''}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setFormData({ name: '', description: '', duration: '', classId: '' });
        ...
        toast.error(`Failed to create Model Test: ${data.error || 'Unknown error'}`, {
          position: "top-right",
          autoClose: 5000,
        });
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          {/* Form */}
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">Model Test Details</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Physics Model Test 1"
                  required
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all"
                  rows={3}
                  placeholder="Brief description of the model test"
                  required
                />
              </div>
              <div>
              ...
              >
                ...
                {isLoading ? 'Creating...' : 'Create Model Test'}
              </button>
            </form>
          </div>
        </div>
