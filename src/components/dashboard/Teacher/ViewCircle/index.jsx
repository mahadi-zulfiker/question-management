...
export default function ViewCircle() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        console.log("Fetched circles:", circlesRes.data);
        console.log("Fetched students:", studentsRes.data);
        setCircles(circlesRes.data);
        setStudents(studentsRes.data.filter(student => student._id && typeof student._id === "string"));
        setLoading(false);
      } catch (error) {
  ...
  return (
    <div className="max-w-5xl mx-auto p-8 bg-gradient-to-b from-blue-50 to-gray-100 min-h-screen">
      ...
      {circles.length === 0 ? (
        <p className="text-center text-gray-500">কোনো সার্কেল নেই।</p>
      ) : (
        <motion.div
            ...
            >
              ...
              <div className="mb-4">
                ...
                {circle.studentIds.length > 0 ? (
                  <ul className="space-y-2">
                    {circle.studentIds
                      .filter((studentId) => studentId && typeof studentId === "string")
                      .map((studentId) => {
                        ...
                        return (
                          ...
                          >
                            <span className="text-gray-800">
                              {student ? `${student.username} (${studentId.slice(-6)})` : `Unknown (${studentId.slice(-6)})`}
                            </span>
                            <button
              ...
              <div className="flex items-center mb-4">
                ...
                >
                  ...
                  {students
                    .filter((student) => student._id && typeof student._id === "string" && !circle.studentIds.includes(student._id))
                    .map((student) => (
