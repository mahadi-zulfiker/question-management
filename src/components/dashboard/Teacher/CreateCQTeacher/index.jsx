...
// Deserialize HTML to Slate (optional, if needed for future editing)
const deserializeFromHtml = (html) => {
  ...
  const deserializeNode = (node) => {
    ...
    if (node.nodeName === "P") return { type: "paragraph", children: children.length ? children : [{ text: "" }] };
    if (node.nodeName === "H1") return { type: "heading-one", children: children.length ? children : [{ text: "" }] };
    if (node.nodeName === "H2") return { type: "heading-two", children: children.length ? children : [{ text: "" }] };
    if (node.nodeName === "H3") return { type: "heading-three", children: children.length ? children : [{ text: "" }] };
    if (node.nodeName === "UL") return { type: "bulleted-list", children: children.length ? children : [{ text: "" }] };
    if (node.nodeName === "OL") return { type: "numbered-list", children: children.length ? children : [{ text: "" }] };
    if (node.nodeName === "LI") return { type: "list-item", children: children.length ? children : [{ text: "" }] };
    if (node.nodeName === "SPAN" && node.className === "mathjax") {
      const text = node.textContent.replace(/\\\(|\\/g, "");
      return { math: true, text };
    }
    if (node.nodeName === "STRONG") return { bold: true, text: node.textContent };
    if (node.nodeName === "EM") return { italic: true, text: node.textContent };
    if (node.nodeName === "U") return { underline: true, text: node.textContent };
    if (node.nodeName === "DEL") return { strikethrough: true, text: node.textContent };
...
export default function CreateCQTeacher() {
        ...
        if (data.length > 0) {
          ...
          data.forEach((item) => {
            ...
            if (!chapterMap.has(key)) {
              chapterMap.set(key, { number: item.chapterNumber, name: item.chapterName });
            }
          });
      ...
      try {
        ...
        if (data.length > 0) {
          const extractedQuestions = data.map((row) => ({
            ...
            teacherEmail, // Include teacher email from session
          }));
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("teacherEmail", teacherEmail); // Use session email
    formData.append("cqType", cqType);

    cqs.forEach((cq, index) => {
      const passageHtml = serializeToHtml(cq.passage);
      const questionsHtml = (cqType === "generalCQ" ? cq.questions : cq.mathQuestions).map((q) => serializeToHtml(q));
      const answersHtml = (cqType === "generalCQ" ? cq.answers : cq.mathAnswers).map((a) => serializeToHtml(a));
    ...
    <>
      <Head>
        ...
        <style>{`
          ...
          .slate-editor { border: 1px solid #d1d5db; border-radius: 0.375rem; margin-bottom: 1rem; }
          .slate-editor .toolbar { border-bottom: 1px solid #d1d5db; background-color: #f7fafc; border-top-left-radius: 0.375rem; border-top-right-radius: 0.375rem; }
        `}</style>
      </Head>
                  ...
                  {(cqType === "generalCQ" ? cq.questions : cq.mathQuestions).map((ques, i) => (
                    <div key={i} className="mb-2">
                      <p className="bangla-text">
                        {String.fromCharCode(2453 + i)}) <span dangerouslySetInnerHTML={{ __html: serializeToHtml(ques) || "প্রশ্ন লিখুন" }} />{" "}
                      ...
                      {(cqType === "generalCQ" ? cq.answers[i] : cq.mathAnswers[i]) && (
                        <p className="text-gray-600 ml-4 bangla-text">
                          ...
                          <span dangerouslySetInnerHTML={{ __html: serializeToHtml(cqType === "generalCQ" ? cq.answers[i] : cq.mathAnswers[i]) }} />
                        </p>
                      )}
                    </div>
                  ))}
